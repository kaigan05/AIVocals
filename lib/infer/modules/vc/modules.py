...
class VC:
    ...
    def vc_single(
        ...
        try:
            ...
            end_time = time.time()
            total_time = end_time - start_time
            # output_dir = "assets/audios/audio-outputs"
            os.makedirs(output_dir, exist_ok=True)
            output_count = 1
            file_name = os.path.basename(input_audio_path1).split('.')[0]
            opt_filename = f"{file_name}_{f0_method}_Conver.{format1}"
            current_output_path = os.path.join(output_dir, opt_filename)
            print(current_output_path)
            while True:
                opt_filename = f"{file_name}_{f0_method}_Conver_{output_count}.{format1}"
                current_output_path = os.path.join(output_dir, opt_filename)
