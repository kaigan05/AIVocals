...
def run_mdx(onnx, mdx_model,filename, output_dir, output_format='wav',diff=False,suffix=None,diff_suffix=None, denoise=False, m_threads=2):
    ...
    print(f"mdx_model.stem_name {mdx_model.stem_name}")
    ...
    vocals_save_path = os.path.join(output_dir, f"{save_path}_{stem_name}.{output_format}")
    # instrumental_save_path = os.path.join(output_dir, f"{save_path}_{stem_name}.{output_format}")
    ...
    print(f'Done, saved to: {vocals_save_path}')
    ...
    if diff:
        ...
        #     )
        sf.write(
            instrumental_save_path,
            (-wave_processed.T*mdx_model.compensation)+wave.T,
            sr
        )
        print(f'Invert done, saved to: {instrumental_save_path}')
    del mdx_sess, wave_processed, wave
    gc.collect()
    return vocals_save_path,instrumental_save_path
